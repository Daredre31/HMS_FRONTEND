...
export default function Rooms() {
  ...
  const [form, setForm] = useState({ roomNumber: "", roomCapacity: "" });
  ...
  const handleCreateRoom = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!form.roomNumber || !form.roomCapacity) {
  ...
  const statusCounts = {
    ...
    full: rooms.filter((r) => r.roomStatus === "fullyOccupied").length,
    maintenance: rooms.filter((r) => r.roomStatus === "under_Maintenance").length,
  };
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 bg-bg-card border-b border-border flex items-center justify-between px-4 md:px-6 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-semibold text-text-primary tracking-tight">Rooms</h1>
              <p className="text-xs text-text-secondary hidden sm:block">{rooms.length} rooms total</p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6">
          ...
          {!loading && rooms.length > 0 && (
            <div className="grid grid-cols-3 gap-3 mb-6">
              {[
                { label: "Available", count: statusCounts.available, color: "bg-green-bg text-green border-green-border" },
                { label: "Full", count: statusCounts.full, color: "bg-red-bg text-red border-red-border" },
                { label: "Maintenance", count: statusCounts.maintenance, color: "bg-amber-bg text-amber border-amber-border" },
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 animate-pulse">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="bg-bg-card rounded-xl border border-border h-36" />
              ))}
            </div>
          ) : rooms.length === 0 ? (
            <div className="bg-bg-card rounded-xl border border-border py-16 text-center">
              ...
              <p className="text-sm font-medium text-text-primary mb-1">No rooms yet</p>
              <p className="text-xs text-text-muted mb-4">Create your first room to start assigning beds and students.</p>
              <button
                onClick={() => setShowModal(true)}
              ...
              >
                <PlusIcon /> Add first room
              </button>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {rooms.map((room) => (
                <div key={room._id} className="bg-bg-card rounded-xl border border-border p-5 hover:border-teal transition-colors group">
                  {/* Room header */}
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-xs text-text-muted mt-0.5">
                        Capacity: {room.roomCapacity} beds
                      </p>
                    </div>
                    <RoomStatusBadge status={room.roomStatus} />
                  </div>

                  {/* Capacity bar — gives a quick visual of how full the room is */}
                  <div className="mb-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-bg-card rounded-2xl border border-border w-full max-w-sm shadow-xl">
            <div className="flex items-center justify-between p-5 border-b border-border">
              <h2 className="text-sm font-semibold text-text-primary">Add new room</h2>
            ...
            <form onSubmit={handleCreateRoom} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-text-primary mb-1.5">Room number</label>
                <input
                  type="text" name="roomNumber" placeholder="e.g. A1, 101, Block-B"
              ...
              <div>
                <label className="block text-xs font-medium text-text-primary mb-1.5">Bed capacity</label>
                <input
                  type="number" name="roomCapacity" placeholder="e.g. 4" min="1" max="20"
                  value={form.roomCapacity} onChange={handleChange}
                  className="w-full px-3 py-2.5 rounded-lg text-sm bg-bg-page border border-border text-text-primary placeholder:text-text-muted focus:outline-none focus:border-teal transition-colors"
                />
                <p className="text-xs text-text-muted mt-1">
                  You'll add the individual beds after creating the room.
                </p>
              </div>
              ...
              <div className="flex gap-3 pt-1">
                ...
                >
                  {creating ? <><SpinnerIcon />Creating...</> : "Create room"}
                </button>
              </div>
            </form>
          </div>
